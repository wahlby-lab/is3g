...
class SiameseNet(nn.Module):
    ...
    def score_edge(self, x:torch.tensor, y:torch.tensor, device:str='cpu', attractive:bool=True):
        z = self.forward(x,y)
...
def istseg(xy: np.ndarray, labels: np.ndarray, cell_diameter: float, remove_background: bool = True,
    ...
    if labels_to_ignore_in_kde is not None and not isinstance(labels_to_ignore_in_kde, Sequence):
    ...
    if return_edges:
        cluster, signed_edges, active_set = _cluster(xy_filt, labels_filt, cell_diameter, labels_to_ignore_in_kde)
        ...
        cluster, _, _ = _cluster(xy_filt, labels_filt, cell_diameter, labels_to_ignore_in_kde)
        cluster_labels[ind] = cluster
        cluster_labels = cluster_labels + 1
...
if __name__ == '__main__':
    ...
    if experiment == 'simulated':
        ...
        for cell_diameter in [35,45,55,65]:
            ...
            data.to_csv(os.path.join('datasets','christophe',f'result_{cell_diameter}.csv'))
        ...
        for cell_diameter in [100]:
            ...
            # Run the clustering
            clusters, signed_edges, active_set = istseg(xy, labels,cell_diameter=cell_diameter, labels_to_ignore_in_kde=['cell_center'])

            # Add to results
            data['instances'] = clusters
            data.to_csv(os.path.join('datasets','osmFISH',f'osmFISH_cropped_result{cell_diameter}.csv'))
    elif experiment == 'osmFISH-full': 
        for cell_diameter in [75,100,125,150,175]:
            # Load the data
            data = pd.read_csv(os.path.join('datasets','osmFISH','osmFISH_all_data+methods.csv'))
            ...
            # Add to results
            data['instances'] = clusters
            data.to_csv(os.path.join('datasets','osmFISH','results', f'osmFISH_full_result{cell_diameter}.csv'))
        ...
        for cell_diameter in [30,50,70,90]:
            ...
            # Add to results
            data['instances'] = clusters
            data.to_csv(os.path.join('datasets','iss','results', f'iss_full_result{cell_diameter}.csv'))